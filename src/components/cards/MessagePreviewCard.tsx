...
export function MessagePreviewCard({ conversation, currentUserId }: { conversation: ConversationModel; currentUserId: string }) {
  ...
  return (
    <Link to={`/app/messages/${conversation.id}`} className="message-preview-card">
      <svg className="message-preview-card__wave" viewBox="0 0 1440 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      ...
      <div className="message-preview-card__avatar-wrap">
        <img src={target?.photoUrl} alt={target?.displayName} className="message-preview-card__avatar" />
      </div>
      ...
      <div className="message-preview-card__chevron-wrap">
        <ChevronRight className="message-preview-card__chevron" size={18} />
      </div>
    </Link>
  );
}
