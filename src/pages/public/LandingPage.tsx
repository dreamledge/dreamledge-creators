...
export function LandingPage() {
  ...
  useEffect(() => {
    const timerId = window.setInterval(() => {
      setActiveTweetIndex((current) => (current + 1) % showcaseTweets.length);
    }, 3600);
  ...
  return (
    <div className="relative -mt-[60px] min-h-screen overflow-hidden bg-black text-white">
      <div className="fixed inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/28 via-black/52 to-black/92" />
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute left-0 top-0 h-56 w-full bg-gradient-to-b from-red-500/22 to-transparent" />
          <div className="absolute right-0 top-24 h-52 w-52 rounded-[999px] bg-red-500/12 blur-3xl" />
          <div className="absolute -left-12 bottom-24 h-52 w-52 rounded-[999px] bg-red-300/10 blur-3xl" />
        </div>
      </div>
      ...
      <div className="relative z-10 min-h-screen w-full overflow-visible pb-10">
          ...
          <div className="mx-auto flex h-[70px] w-full max-w-7xl items-center px-5 sm:px-6 lg:px-8">
            <div className="relative mx-auto grid w-full max-w-sm grid-cols-[48px_1fr_auto] items-center gap-4 lg:max-w-none lg:grid-cols-[48px_1fr_auto]">
              <div className="flex h-11 w-11 items-center justify-center rounded-[999px] border border-white/8 bg-white/[0.04]">
                <div className="flex h-7 w-7 items-center justify-center rounded-[999px] bg-[linear-gradient(180deg,rgba(255,91,91,0.92),rgba(255,45,61,0.92))] text-sm font-bold text-[#F5F5F7]">
                  D
                </div>
              </div>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 pt-28 sm:px-6 lg:px-8">
          <div className="mx-auto w-full max-w-none">
            <LandingRevealSection className="landing-hero-reveal mt-6 lg:mt-10">
              <div className="landing-ig-hero-grid">
                <div className="landing-ig-showcase landing-reveal-item order-2 lg:order-1">
                  <div className="landing-ig-phone-shell">
                    <div className="landing-ig-phone-notch" />
                    <div className="landing-ig-phone-screen">
                      <div className="landing-ig-tweet-stage">
                        {showcaseTweets.map((item, index) => (
                          <article
                            key={item.handle}
                            className={`landing-ig-tweet-card ${index === activeTweetIndex ? "is-active" : "is-idle"}`}
                            aria-hidden={index !== activeTweetIndex}
                          >
                            <header className="landing-ig-tweet-head">
                              <img src={item.image} alt={item.name} className="landing-ig-tweet-avatar" />
                              <div className="landing-ig-tweet-user">
                                <p>{item.name}</p>
                                <span>{item.handle}</span>
                              </div>
                              <span className="landing-ig-tweet-badge">Tweet</span>
                            </header>
                            <p className="landing-ig-tweet-body">{item.tweet}</p>
                            <footer className="landing-ig-tweet-metrics">{item.metrics}</footer>
                          </article>
                        ))}
                      </div>

                      <div className="landing-ig-tweet-dots" aria-label="Tweet carousel controls">
                        {showcaseTweets.map((item, index) => (
                          <button
                            key={item.handle}
                            type="button"
                            className={`landing-ig-tweet-dot ${index === activeTweetIndex ? "is-active" : ""}`}
                            onClick={() => setActiveTweetIndex(index)}
                            aria-label={`Show tweet from ${item.name}`}
                            aria-pressed={index === activeTweetIndex}
                          />
                        ))}
                      </div>
                    </div>
                  </div>
                  <div className="landing-ig-float-badge landing-ig-float-badge-left">
                    <span>Weekly Battles</span>
                    <strong>+18% visibility</strong>
                  </div>
                  <div className="landing-ig-float-badge landing-ig-float-badge-right">
                    <span>Live Rankings</span>
                    <strong>2.4k creators active</strong>
                  </div>
                </div>

                <div className="landing-ig-copy order-1 text-center lg:order-2">
                  <p className="landing-reveal-item landing-ig-kicker">Creators-first platform</p>
                  <h2 className="landing-reveal-item text-[2rem] font-bold leading-[0.95] tracking-tight text-white sm:text-5xl lg:text-6xl">
                  ...
                  <div className="landing-reveal-item mx-auto mt-4 max-w-[32rem] space-y-2 text-sm leading-6 text-zinc-300 sm:text-base">
                  ...
                  <div className="landing-reveal-item mt-7 flex justify-center">
                    <Link to="/signup" className="landing-cta-button landing-cta-primary landing-ig-hero-cta w-full sm:w-auto">
                  ...
                  <div className="landing-reveal-item landing-ig-proof-grid text-center">
                    <div>
                      <p>50k+</p>
                      <span>Weekly ratings</span>
                    </div>
                    <div>
                      <p>4.8/5</p>
            ...
            <LandingRevealSection className="landing-features-reveal mt-12">
              <div className="landing-ig-feature-grid">
                {features.map((feature, index) => (
                  <article key={feature.title} className="landing-feature-card landing-reveal-item" style={{ transitionDelay: `${index * 120}ms` }}>
            ...
            <LandingRevealSection className="landing-lower-reveal mt-10">
              <div className="landing-reveal-item lg:grid lg:grid-cols-[1.05fr_0.95fr] lg:gap-8">
                <div>
                  <div className="flex items-center justify-between">
                    ...
                    <button className="landing-ig-link-button text-xs font-medium text-red-300">View all</button>
                  </div>
                  <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden lg:flex-wrap lg:overflow-visible">
                    {categories.map((category, index) => (
                      <span
                        key={category}
                        className="landing-category-chip landing-reveal-item whitespace-nowrap rounded-[999px] px-3.5 py-2 text-xs"
                ...
                <div className="landing-reveal-item mt-8 lg:mt-0">
                  <div className="landing-ig-join-card rounded-[30px] p-5 sm:p-6">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.2em] text-zinc-300/85">Join Dreamledge</p>
                    <h3 className="mt-2 text-2xl font-bold tracking-tight text-white">Build your audience and compete for attention.</h3>
                    ...
                    <Link
                      to="/signup"
                      className="landing-ig-create-account-btn mt-5 flex w-full items-center justify-center rounded-[28px] bg-white/95 px-4 py-3.5 text-center text-sm font-semibold shadow-lg transition hover:bg-white"
                    >
                      Create account
                    </Link>
                  </div>
              ...
              <footer className="landing-reveal-item mt-14 pt-8">
                <div>
                  ...
                  <p className="mt-8 pb-2 text-center text-xs tracking-[0.18em] text-white/38">© 2026 Dreamledge. All rights reserved.</p>
                </div>
              </footer>
            </LandingRevealSection>
          </div>
